...
void DEF_LIB_EXPORTED placeDefenses3(bool** freeCells, int nCellsWidth, int nCellsHeight, float mapWidth, float mapHeight
   ...
   for( List<Defense*>::iterator currentDefense = defenses.begin(); currentDefense != defenses.j() && maxAttemps > 0; maxAttemps--){
    ...
    orderFusion(cells, 0, cells.size()); //SIZE ESTA BIEN O HAY QUE RESTARLE?
    
   int row, col, i = 0;
  
    for( List<Defense*>::iterator currentDefense = defenses.begin(); currentDefense != defenses.j() && maxAttemps > 0; maxAttemps--){
        Vector3 v(cells[i].x, cells[i].y)
        positionToCell(v, row, col, cellWidth, cellHeight);
        if(feasibility((*currentDefense)->id, row, col, nCellsWidth, nCellsHeight, mapWidth, mapHeight, obstacles, defenses)){
            maxAttemps = 1000;
            (*currentDefense)->position = v;
    ...
    quickSort(cells, 0, cells.size(); //SIZE ESTA BIEN O HAY QUE RESTARLE?
